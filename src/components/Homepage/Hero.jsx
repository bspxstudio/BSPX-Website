...
// Impor gambar Mobile (dari folder gallery sesuai screenshot)
...
const slides = [
  {
    ...
    mobileImg: MobileImg1, // Gambar untuk Mobile
  ...
  },
  {
    ...
    img: BirthdayShoots,
    mobileImg: MobileImg2, // Gambar untuk Mobile
  ...
  },
  {
    ...
    img: CoupleShoots,
    mobileImg: MobileImg3, // Gambar untuk Mobile
  ...
  },
  {
    ...
    img: PrePostWedding,
    mobileImg: MobileImg4, // Gambar untuk Mobile
...
export default function Hero() {
  ...
  return (
    <section className="relative w-full overflow-hidden bg-[#0a0a0a] text-white selection:bg-[#D4A53A] selection:text-black">
      <div className="relative min-h-[100svh] w-full overflow-hidden flex flex-col justify-center">
        ...
        <div className="absolute inset-0 z-0">
          {slides.map((slide, index) => (
            ...
            >
              {/* Menggunakan tag picture untuk memisahkan gambar mobile dan desktop */}
              <picture>
                {/* Tampilkan gambar desktop jika ukuran layar minimal 768px (md) */}
                <source media="(min-width: 768px)" srcSet={slide.img} />
                {/* Tampilkan gambar mobile sebagai fallback/default untuk layar kecil */}
                <img
                  src={slide.mobileImg}
          ...
          <div className="w-full max-w-3xl xl:max-w-4xl">
            ...
            <h1 className="whitespace-pre-line font-['Helvetica_Neue',_Helvetica,_Arial,_sans-serif] text-5xl min-[380px]:text-6xl sm:text-7xl md:text-8xl lg:text-[5.5rem] xl:text-[6.5rem] font-bold leading-[1.05] tracking-tighter text-white mb-6 drop-shadow-2xl uppercase">
              {activeSlide.title.split('\n').map((line, i) => (
                <span key={i} className={`block ${i === 1 ? 'text-white/80' : 'text-white'}`}>
                  {line}
                </span>
              ))}
            </h1>
            ...
            <div className="flex flex-col sm:flex-row gap-6 sm:gap-12 mb-12">
              <div className="group flex items-start gap-3">
                <MapPin className="h-5 w-5 text-[#D4A53A] mt-0.5 transition-transform group-hover:-translate-y-1" strokeWidth={1.5} />
                <div>
                  <p className="font-sans text-[9px] font-bold uppercase tracking-[0.2em] text-white/50 mb-1">Location</p>
                  <p className="font-sans text-sm tracking-wide text-white/90">{activeSlide.location}</p>
                </div>
              </div>
              <div className="hidden sm:block w-[1px] h-10 bg-white/20" />
              <div className="group flex items-start gap-3">
                <Phone className="h-5 w-5 text-[#D4A53A] mt-0.5 transition-transform group-hover:-translate-y-1" strokeWidth={1.5} />
                <div>
                  <p className="font-sans text-[9px] font-bold uppercase tracking-[0.2em] text-white/50 mb-1">Inquiries</p>
                  <p className="font-sans text-sm tracking-wide text-white/90">+91 8531985733</p>
                </div>
              </div>
            </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 z-40 bg-gradient-to-t from-black to-transparent pt-20 pb-6 px-6 sm:px-12 lg:px-20">
          <div className="mx-auto max-w-[1800px] flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <div className="flex items-center gap-4">
              ...
              >
                <ChevronLeft className="h-5 w-5 text-white transition-colors group-hover:text-black" strokeWidth={1.5} />
              </button>
              <button
                onClick={nextSlide}
                className="group flex h-12 w-12 items-center justify-center rounded-full border border-white/20 backdrop-blur-md transition-all duration-300 hover:border-white hover:bg-white"
              >
                <ChevronRight className="h-5 w-5 text-white transition-colors group-hover:text-black" strokeWidth={1.5} />
              </button>
            ...
            <div className="flex items-center gap-4 w-full md:w-auto md:min-w-[400px] lg:min-w-[600px]">
              {slides.map((slide, index) => (
                ...
                >
                  <span className={`text-left font-sans text-[9px] font-bold uppercase tracking-[0.1em] mb-3 transition-all duration-300 ${
                    current === index ? 'text-white opacity-100 translate-y-0' : 'text-white/40 opacity-0 translate-y-2 group-hover:opacity-100'
                  }`}>
                    {slide.shortTitle}
                  </span>
